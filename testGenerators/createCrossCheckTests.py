...
for i, test_case in enumerate(raw_inputs.split("\n")):
    ...
    template1 = (
        f"@Test\n"
        f"public void test{i + 1}CCrossCheckCompress_{decoded_string}() {{\n"
        f"    String input = \"{decoded_string}\";\n"
        f"    assertEquals(AlternativeRLE.runLengthEncode(input), StringRLEAlgorithm.compress(input));\n"
        f"}}\n\n"
    )
    ...
    template2 = (
        f"@Test\n"
        f"public void test{i + 1}DCrossCheckDecompress_{encoded_string}() {{\n"
        f"    String input = \"{encoded_string}\";\n"
        f"    assertEquals(AlternativeRLE.runLengthDecode(input), StringRLEAlgorithm.decompress(input));\n"
        f"}}\n\n"
    )
